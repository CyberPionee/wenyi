...
from trans_novel.cli import create_app, main
from trans_novel.commands.context import current_context
...
def test_help_paths_and_overrides_do_not_leak_between_invocations(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("trans_novel.llm.factory.build_client", lambda config: Client())
...
def test_installed_entry_point_runs_real_bootstrap_before_help(tmp_path, monkeypatch, capsys):
    entry = next(e for e in distribution("trans-novel").entry_points if e.name == "trans-novel")
    assert entry.value == "trans_novel.cli:main"
