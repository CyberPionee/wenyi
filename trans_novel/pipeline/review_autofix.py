...
from .autofix_plan import load_json, prepare_identity, save_plan
...
class ReviewAutofixService:
    ...
    def _review_result(run_dir: str) -> dict[str, Any] | None:
        ...
        try:
            result = load_json(os.path.join(run_dir, "result.json"))
    ...
    def _index(run_dir: str) -> dict[str, Any] | None:
        ...
        try:
            index = load_json(os.path.join(run_dir, "autofix", "index.json"))
        except (OSError, json.JSONDecodeError, TypeError):
            return None
